...
export function getSelectedAddress(payload){
	...
	return(dispatch, store)=>{
		...
		.then(()=>{
          ...
          key:"AIzaSyDu744m9kuAZCFCVumXig33497gsWpKPs0"
				})
				.finish((error, res)=>{
					dispatch({
...
//console.log("Number of nearByDrivers", nearByDrivers.length);

//BOOK CAR

export function bookCar(){
 console.log("In bookCar");
	return (dispatch, store)=>{
		const nearByDrivers = store().home.nearByDrivers;
    console.log("Number of nearByDrivers", nearByDrivers.length);
		const nearByDriver = nearByDrivers[Math.floor(Math.random() * nearByDrivers.length)];
    //const nearByDriver = nearByDrivers[Math.floor(Math.random() * 2)];
		const payload = {
			data:{
				userName:"yehdhih",
				pickUp:{
		...
		//request.post("http://24.28.21.64:3000/api/bookings")
    //request.post("http://192.168.0.4:3000/api/bookings")
    //request.post("http://localhost:3000/api/bookings")
    request.post("http://35.231.160.106:3000/api/bookings")
		.send(payload)
		.finish((error, res)=>{
			dispatch({
				type:BOOK_CAR,
...
export function getNearByDrivers(){
	return(dispatch, store)=>{
    //request.get("http://24.28.21.64:3000/api/driverLocation")
    //request.post("http://192.168.0.4:3000/api/driverLocation")
    //request.get("http://localhost:3000/api/driverLocation")
    request.get("http://35.231.160.106:3000/api/driverLocation")
		.query({
          //latitude:3.145909,
          //longitude:101.696985
			//latitude:18.0939,
